  ...
  <td width='750px' height="300" align="center" valign=top style="padding:20px 10px 0 10px;">
   ...
   <div align="center" style="margin: 0 0 25px 0; padding: 2px 2px 2px 2px;"></div>  
	...
	<div align='left' class='divTitulo'>
		<span class='titulo'>.: Edição de Supervisor</span>
...
if($submit){
	...
	$q_repetido = "SELECT * FROM supervisores WHERE cpf = '{$cpf}' AND id <> '{$id}';";
	...
	if(count($erros)>0){
		...
		if(count($erros)==1)
			if($erros[0] == "cpf" && !empty($cpf))
				$msg_erro = "Outro supervisor com esse CPF já existe no sistema.";
			else				
		...
		$query = "UPDATE supervisores SET 
		nome = '{$nome}', formacao = '{$formacao}', cargo = '{$cargo}', cpf = '{$cpf}' WHERE id = {$id};";			
		$result = sql_executa($query);
		//mensagem de sucesso		
		if($result){
			echo "<table width='100%' style='border:1px solid black;' bgcolor='' cellspacing='0' cellpadding='5' height='50px'>						
				<tr bgcolor='#EFFFF4'>
					<td align='center'><span align='center' style='color:#296F3E;'>Supervisor editado com sucesso!</span></td>
				</tr>
			</table>		
			<div align='center' style='margin: 0 0 25px 0; padding: 2px 2px 2px 2px;'></div>";
	...
	if($id){
		//Recuperando dados do supervisor
		$q_recupera = "SELECT * FROM supervisores WHERE id = {$id}";
