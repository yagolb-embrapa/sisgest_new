...
$id = $_GET['id'];	
$cor = 0;	
$qryStr = "SELECT nome FROM supervisores WHERE id = {$id}";
...
$q_delete = "DELETE FROM supervisores WHERE id = {$id};";
$r_delete = sql_executa($q_delete);

if($r_delete){
	$msg = "O surpevisor {$row['nome']} foi excluído com sucesso!";
}else{
	$msg = "Não foi possível excluir o supervisor {$row['nome']}. Por favor, tente novamente.";
}
