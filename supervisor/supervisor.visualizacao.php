  ...
  <td width='752' height="300" align="center" valign=top style="padding:20px 10px 0 10px;">
   ...
   <div align="center" style="margin: 0 0 25px 0; padding: 2px 2px 2px 2px;"></div>  
	...
	<div class='divTitulo' align='left'>
		<span class='titulo'>.: Visualização de Supervisor</span>
		<div align="center" style="margin: 0 0 25px 0; padding: 2px 2px 2px 2px;"></div>
...
if(empty($id))
	$msg_erro = "O supervisor não foi encontrado.";		
else{		
	$query_superv = "SELECT * FROM supervisores WHERE id = {$id}";
...
if($msg_erro){
    ...
    echo "<p><a href='javascript://' onclick=\"document.location.href='supervisor.edicao.php?id=".$id."';\">
   ...
   <!-- Abas -->	
	...
	<div id="aba1" class='conteudoAba' style='display:block;'>		  	 	
      ...
      <tr class='specalt'>
        ...
        <td width="67%"><span><?php echo (empty($campo['cpf']))?" <i>Não preenchido</i> ":$campo['cpf']; ?></span></td>        
      </tr>
		<tr class='specalt'>
        <td ><span>Formação</span></td>
        <td><span><?php echo (empty($campo['formacao']))?" <i>Não preenchido</i> ":$campo['formacao']; ?></span></td>
       </tr>
       <tr class='specalt'>
        <td ><span>Cargo</span></td>
        <td><span><?php echo (empty($campo['cargo']))?" <i>Não preenchido</i> ":$campo['cargo']; ?></span></td>
       </tr>
