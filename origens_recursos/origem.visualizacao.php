  ...
  <td width='752' height="300" align="center" valign=top style="padding:20px 10px 0 10px;">
   ...
   <div align="center" style="margin: 0 0 25px 0; padding: 2px 2px 2px 2px;"></div>  
	...
	<div class='divTitulo' align='left'>
		<span class='titulo'>.: Visualização da Origem de Recursos</span>
		<div align="center" style="margin: 0 0 25px 0; padding: 2px 2px 2px 2px;"></div>
...
if(empty($id))
	$msg_erro = "A origem de recursos não foi encontrado.";		
else{		
	$query_superv = "SELECT * FROM origens_recursos WHERE id = {$id}";
	$result_superv = sql_executa($query_superv);	
	if(sql_num_rows($result_superv)==0)
		$msg_erro = "A origem de recursos não foi encontrada.";			
	else
...
if($msg_erro){
    ...
    echo "<p><a href='javascript://' onclick=\"document.location.href='origem.edicao.php?id=".$id."';\">
   ...
   <!-- Abas -->	
	...
	<div id="aba1" class='conteudoAba' style='display:block;'>		  	 	
      ...
      <tr class='specalt'>
        ...
        <td width="67%"><span><?php echo (empty($campo['origem']))?" <i>Não preenchido</i> ":$campo['origem']; ?></span></td>        
      </tr>
