  ...
  <td width='750px' height="300" align="center" valign=top style="padding:20px 10px 0 10px;">
   ...
   <div align="center" style="margin: 0 0 25px 0; padding: 2px 2px 2px 2px;"></div>  
	...
	<div align='left' class='divTitulo'>
        <span class='titulo'>.: Edição de Origem de Recursos</span>
...
if($submit){
	...
	$q_repetido = "SELECT * FROM origens_recursos WHERE origem = '{$origem}' AND id <> '{$id}';";
	...
	if(count($erros)>0){
		...
		if(count($erros)==1)
			if($erros[0] == "origem" && !empty($origem))
				$msg_erro = "Outra origem com esse nome já existe no sistema.";
			else				
			...
			if(in_array("codigo",$erros) && !empty($codigo))
				$msg_erro = "Outro supervisor com esse CPF já existe no sistema.<br>Alguns campos não foram preenchidos corretamente e foram marcados com um asterisco vermelho. Por favor, verifique-os e tente novamente.";
			else	
		...
		if($result){
			echo "<table width='100%' style='border:1px solid black;' bgcolor='' cellspacing='0' cellpadding='5' height='50px'>						
				<tr bgcolor='#EFFFF4'>
					<td align='center'><span align='center' style='color:#296F3E;'>Origem de Recursos editada com sucesso!</span></td>
				</tr>
			</table>		
			<div align='center' style='margin: 0 0 25px 0; padding: 2px 2px 2px 2px;'></div>";
	...
	if($id){
		//Recuperando dados da origem de recursos
		$q_recupera = "SELECT * FROM origens_recursos WHERE id = {$id}";
		...
		if(sql_num_rows($r_recupera)>0){
			...
			echo "Origem de Recursos não encontrada";		
		}						
	}
}
?>
   <!-- Abas -->	
	...
	<div id="aba1" class='conteudoAba' style='display:block;'>
      ...
      <tr class='specalt'>
        ...
        <td width="75%"><input name="origem" id="origem" type="text" size='40' maxlength='50' value="<?php echo $origem; ?>"><span id='sorigem' class="sErro">&nbsp;*</span></td>        
      </tr>
       <tr><td>&nbsp;</td><td>&nbsp;</td></tr>
       </table>
       </div>  
