  ...
  <td width='750px' height="300" align="center" valign=top style="padding:20px 10px 0 10px;">
   ...
   <div align="center" style="margin: 0 0 25px 0; padding: 2px 2px 2px 2px;"></div>  
	...
	<div align='left' class='divTitulo'>
		<span class='titulo'>.: Inclusão de Email</span>
...
if($submit){
    ...
    $q_repetido = " SELECT  * 
                    FROM    emails
                    WHERE   id_setor = '{$id_setor}' AND email = '{$email}';";
	$r_repetido = sql_executa($q_repetido);
	if(sql_num_rows($r_repetido)>0) $erros[] = "email";	
	...
	if(count($erros)>0){
		...
		if(count($erros)==1)
			if($erros[0] == "email" && !empty($email))
				$msg_erro = "O email já está cadastrado no sistema.";
			else				
			...
			if(in_array("codigo",$erros) && !empty($codigo))
				$msg_erro = "O email já está cadastrado no sistema.<br>Alguns campos não foram preenchidos corretamente e foram marcados com um asterisco vermelho. Por favor, verifique-os e tente novamente.";
			else	
    ...
    //Coloca um sinal vermelho ao lado dos campos não-preenchidos ou preenchidos de forma incorreta
		...
		$query = "INSERT INTO emails (id_setor, email) VALUES('{$id_setor}', '{$email}');";		
		$result = sql_executa($query);
		//mensagem de sucesso		
		if($result){
			echo "<table width='100%' style='border:1px solid black;' bgcolor='' cellspacing='0' cellpadding='5' height='50px'>						
				<tr bgcolor='#EFFFF4'>
					<td align='center'><span align='center' style='color:#296F3E;'>Email incluído com sucesso!</span></td>
				</tr>
			</table>		
			<div align='center' style='margin: 0 0 25px 0; padding: 2px 2px 2px 2px;'></div>";
   ...
   <!-- Abas -->	
	...
	<div id="aba1" class='conteudoAba' style='display:block;'>
      ...
      <tr class='specalt'>
        ...
        <td width="75%"><input name="email" id="email" type="text" size='60' maxlength='60' value="<?php echo $email; ?>"><span id='semail' class="sErro">&nbsp;*</span></td>        
      </tr>
       <tr><td>&nbsp;</td><td>&nbsp;</td></tr>
       </table>
       </div>  
