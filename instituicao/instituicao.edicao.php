  ...
  <td width='752' height="300" align="center" valign=top style="padding:20px 10px 0 10px;">
   ...
   <div align="center" style="margin: 0 0 25px 0; padding: 2px 2px 2px 2px;"></div>  
	...
	<div class='divTitulo' align='left'>
		<span class='titulo'>.: Edição de Instituição de Ensino</span>
		<div align="center" style="margin: 0 0 25px 0; padding: 2px 2px 2px 2px;"></div>
...
if($submit){
	...
	//$q_repetido = "SELECT * FROM instituicoes_ensino WHERE id <> {$id} AND cnpj = '{$cnpj}';";
	//$r_repetido = sql_executa($q_repetido);
	//if(sql_num_rows($r_repetido)){
		//$repetido = 1;
		//$erros[] = "cnpj";  	
	//}				
	...
	if(count($erros)>0){
		...
		elseif(count($erros) == 1 && $repetido == 1)
			$msg_erro = "O CNPJ digitado para esta instituição já se encontra no sistema. Verifique se a instituição já está cadastrada ou se o CNPJ digitado está correto."; 		
		elseif(count($erros) > 1 && $repetido == 1)
			$msg_erro = "O CNPJ digitado para esta instituição já se encontra no sistema.<br>Alguns campos não foram preenchidos corretamente e foram marcados com um asterisco vermelho. Por favor, verifique-os e tente novamente.";
		else
        ...
        $query = "UPDATE instituicoes_ensino
                  SET    razao_social = '{$razao}', cnpj = '{$cnpj}', endereco = '{$endereco}', 
                         complemento = '{$complemento}', bairro='{$bairro}', cep = '{$cep}', id_municipio = '{$municipio}', uf = '{$uf}',
                         data_convenio = '".formata($data_convenio,'data')."', numero_saic = '{$numero_saic}'
                  WHERE id = {$id};"; 
		...
		if($result){
			echo "<table width='100%' style='border:1px solid black;' bgcolor='' cellspacing='0' cellpadding='5' height='50px'>						
				<tr bgcolor='#EFFFF4'>
					<td align='center'><span align='center' style='color:#296F3E;'>Dados da Instituição editados com sucesso!</span></td>
				</tr>
			</table>		
			<div align='center' style='margin: 0 0 25px 0; padding: 2px 2px 2px 2px;'></div>";			
		}
	...
	if($id){
		//Recuperando dados da instituicao
		$q_recupera = "SELECT * FROM instituicoes_ensino WHERE id = {$id}";
		$r_recupera = sql_executa($q_recupera);
		$c_rec = sql_fetch_array($r_recupera);
		
		if(sql_num_rows($r_recupera)>0){
			extract($c_rec);					
			$razao = $razao_social;
	...
	<div id="aba1" class='conteudoAba' style='display:block;'>
      ...
      <tr class='specalt'>     
       ...
       <tr  class='specalt'>
        ...
        <td><input name="data_convenio" id="data_convenio" type="text" size='10' maxlength='10' value="<?php echo formata($data_convenio,'redata'); ?>"><span id='sdata_convenio' class="sErro">&nbsp;*</span></td>
       </tr>       
