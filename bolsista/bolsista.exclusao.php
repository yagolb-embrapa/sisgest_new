...
$id = $_GET['id'];	
$cor = 0;	
$qryStr = "SELECT nome FROM modalidades_bolsista WHERE id = {$id}";
...
$q_delete = "DELETE FROM modalidades_bolsista WHERE id = {$id};";
$r_delete = sql_executa($q_delete);

if($r_delete){
	$msg = "A modalidade {$row['nome']} foi excluída com sucesso!";
}else{
	$msg = "Não foi possível excluir a modalidade {$row['nome']}. Por favor, tente novamente.";
}
