  ...
  <td width='750px' height="300" align="center" valign=top style="padding:20px 10px 0 10px;">
   ...
   <div align="center" style="margin: 0 0 25px 0; padding: 2px 2px 2px 2px;"></div>  
	...
	<div align='left' class='divTitulo'>
		<span class='titulo'>.: Inclusão de Modalidade</span>
...
if($submit){
	...
	$q_repetido = "SELECT * FROM modalidades_bolsista WHERE nome = '{$nome}';";
	...
	if(count($erros)>0){
		...
		if(count($erros)==1)
			if($erros[0] == "nome" && !empty($nome))
				$msg_erro = "A modalidade já está cadastrada no sistema.";
			else				
			...
			if(in_array("codigo",$erros) && !empty($codigo))
				$msg_erro = "A modalidade já está cadastrada no sistema.<br>Alguns campos não foram preenchidos corretamente e foram marcados com um asterisco vermelho. Por favor, verifique-os e tente novamente.";
			else	
		...
		$query = "INSERT INTO modalidades_bolsista (nome) VALUES('{$nome}');";		
		$result = sql_executa($query);
		//mensagem de sucesso		
		if($result){
			echo "<table width='100%' style='border:1px solid black;' bgcolor='' cellspacing='0' cellpadding='5' height='50px'>						
				<tr bgcolor='#EFFFF4'>
					<td align='center'><span align='center' style='color:#296F3E;'>Modalidade incluída com sucesso!</span></td>
				</tr>
			</table>		
			<div align='center' style='margin: 0 0 25px 0; padding: 2px 2px 2px 2px;'></div>";
