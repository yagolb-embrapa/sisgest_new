  ...
  <td width='750px' height="300" align="center" valign=top style="padding:20px 10px 0 10px;">
   ...
   <div align="center" style="margin: 0 0 25px 0; padding: 2px 2px 2px 2px;"></div>  
	...
	<div align='left' class='divTitulo'>
		<span class='titulo'>.: Edição de Modalidade</span>
...
if($submit){
	...
	$q_repetido = "SELECT * FROM modalidades_bolsista WHERE nome = '{$nome}' AND id <> '{$id}';";
	$r_repetido = sql_executa($q_repetido);
	if(sql_num_rows($r_repetido)>0) $erros[] = "nome";	
	...
	if(count($erros)>0){
		...
		if(count($erros)==1)
			if($erros[0] == "nome" && !empty($nome))
				$msg_erro = "Outra modalidade com esse nome já existe no sistema.";
			else				
        ...
        $query = "UPDATE modalidades_bolsista
                  SET    nome = '{$nome}'
                  WHERE  id = {$id};";			
		...
		if($result){
			echo "<table width='100%' style='border:1px solid black;' bgcolor='' cellspacing='0' cellpadding='5' height='50px'>						
				<tr bgcolor='#EFFFF4'>
					<td align='center'><span align='center' style='color:#296F3E;'>Modalidade editada com sucesso!</span></td>
				</tr>
			</table>		
			<div align='center' style='margin: 0 0 25px 0; padding: 2px 2px 2px 2px;'></div>";
	...
	if($id){
		//Recuperando dados da modalidade
		$q_recupera = "SELECT * FROM modalidades_bolsista WHERE id = {$id}";
