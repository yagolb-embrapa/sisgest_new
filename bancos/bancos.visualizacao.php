  ...
  <td width='752' height="300" align="center" valign=top style="padding:20px 10px 0 10px;">
   ...
   <div align="center" style="margin: 0 0 25px 0; padding: 2px 2px 2px 2px;"></div>  
	...
	<div class='divTitulo' align='left'>
		<span class='titulo'>.: Visualização de banco</span>
		<div align="center" style="margin: 0 0 25px 0; padding: 2px 2px 2px 2px;"></div>
...
if(empty($id))
	$msg_erro = "O banco não foi encontrado.";		
else{		
	$query_superv = "SELECT * FROM bancos WHERE id = {$id}";
	$result_superv = sql_executa($query_superv);	
	if(sql_num_rows($result_superv)==0)
		$msg_erro = "O banco não foi encontrada.";			
	else
...
if($msg_erro){
    ...
    echo "<p><a href='javascript://' onclick=\"document.location.href='bancos.edicao.php?id=".$id."';\">
   ...
   <!-- Abas -->	
	...
	<div id="aba1" class='conteudoAba' style='display:block;'>		  	 	
      ...
      <tr class='specalt'>
        ...
        <td width="67%"><span><?php echo (empty($campo['banco']))?" <i>Não preenchido</i> ":$campo['banco']; ?></span></td>        
      </tr>
      <tr class='specalt'>
        <td width="33%"><span>Código</span></td>
        <td width="67%"><span><?php echo (empty($campo['codigo_banco']))?" <i>Não preenchido</i> ":$campo['codigo_banco']; ?></span></td>        
      </tr>  
       <tr><td>&nbsp;</td><td>&nbsp;</td></tr>
       </table>
       </div>
