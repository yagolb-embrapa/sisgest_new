...
$id = $_GET['id'];	
$cor = 0;	
$qryStr = "SELECT banco FROM bancos WHERE id = {$id}";
...
$q_delete = "DELETE FROM bancos WHERE id = {$id};";
$r_delete = sql_executa($q_delete);

if($r_delete){
	$msg = "O banco {$row['banco']} foi excluído com sucesso!";
}else{
	$msg = "Não foi possível excluir o banco {$row['banco']}. Por favor, tente novamente.";
}
